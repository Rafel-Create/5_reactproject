...
//1)클라이언트로부터 'id{1,2,3}'값을 요청본문에서 가져옵니다.
    const id = req.body.id;
//2)주어진 id에 해당하는 게시글 정보를 조회합니다
const sqlQuery =
"SELECT BOARD_ID, BOARD_TITLE, BOARD_CONTENT FROM BOARD WHERE BOARD_ID = ?;";
//3)db.query를 사용해서 mysql 데이터베이스에 쿼리를 실행합니다.
db.query(sqlQuery,(err,result) => {
//4)쿼리 실행 후에 에러가 발생하면 적절히 처리하거나 결과를 클라이언트에게 응답으로 보냅니다. <서버/클라이언트 할 때 바로 그 클라이언트>
res.send(result)
})
})

//글쓰기(새로운 글을 게시판에 쓰면 아이디 등이 새로 생기죠?)
app.post("/insert",(req,res) => {
    const title = req.body.title; //새로운 글을 쓰는데 숫자가 자동으로 1 증가하면서 타이틀과 내용을 MySQL에서 record한다.
